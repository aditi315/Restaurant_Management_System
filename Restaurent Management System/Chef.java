...
public class Chef extends JFrame implements ActionListener{
	JLabel userLabel, eNameLabel, addressLabel, salaryLabel;
	JTextField userTF, addressTF, eNameTF, salaryTF;
	JButton refreshBtn, loadBtn, updateBtn, delBtn, backBtn, logoutBtn;
	JPanel panel;
	String userId;
	
	public Chef(String userId)
	{
		super("Restaurant Management System - View Employee-chef");
	...
	public void loadFromDB()
	{
		...
		String query = "SELECT `name`, `address`, `salary` FROM `chef` WHERE `userId`='"+loadId+"';";     
	...
	public void updateInDB()
	{
		...
		String query = "UPDATE chef SET name='"+eName+"',address= '"+address+"', salary = "+salary+" WHERE userId='"+newId+"'";	
        Connection con=null;//for connection
        Statement st = null;//for query execution
		System.out.println(query);
	...
	{
		String newId = userTF.getText();
		String query1 = "DELETE from chef WHERE userId='"+newId+"';";
