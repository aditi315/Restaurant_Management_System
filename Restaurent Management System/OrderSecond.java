...
public class OrderSecond extends JFrame implements ActionListener
{
	...
	JButton logoutBtn,backBtn,doneBtn, cancelBtn;
	 JPanel panel;
	 String userId;
	int tempcount=0;
	...
	public OrderSecond(String userId)
	{
		super("Second Order");
		...
		title = new JLabel("Second Order Details");
		title.setBounds(250, 30, 150, 30);
		panel.add(title);
		
		logoutBtn = new JButton("Logout");
		logoutBtn.setBounds(450, 500, 100, 30);
	...
	public int orderQuantity()
	{
        ...
        try
		{
				...
				while(rs.next())
			{
				...
				if(s.equals("not done")&&nm.equals("Chicken Burger")&&id==2&&tempcount==0)
				{
					q=quantity;
					break;
				}
				else if(s.equals("not done")&&nm.equals("Beef Burger")&&id==2&&tempcount==1)
				{
					q=quantity;
					break;
				}
				else if(s.equals("not done")&&nm.equals("Peperoni Pizza")&&id==2&&tempcount==2)
				{
					q=quantity;
					break;
				}
				else if(s.equals("not done")&&nm.equals("Cheese Pizza")&&id==2&&tempcount==3)
				{
					q=quantity;
					break;
				}
				else if(s.equals("not done")&&nm.equals("Sandwich")&&id==2&&tempcount==4)
				{
					q=quantity;
					break;
				}
				else if(s.equals("not done")&&nm.equals("Set Menu 01")&&id==2&&tempcount==8)
				{
					q=quantity;
					break;
				}
				else if(s.equals("not done")&&nm.equals("Set Menu 02")&&id==2&&tempcount==9)
				{
					q=quantity;
					break;
				}
				else if(s.equals("not done")&&nm.equals("Coke")&&id==2&&tempcount==5)
				{
					q=quantity;
					break;
				}
				else if(s.equals("not done")&&nm.equals("Coffee")&&id==2&&tempcount==6)
				{
					q=quantity;
					break;
				}
				else if(s.equals("not done")&&nm.equals("Water")&&id==2&&tempcount==7)
				{
					q=quantity;
					break;
				}
	...
	public void orderDone()
	{
        ...
        try
		{
			...
			while(rs.next())
			{
				...
				if(s.equals("not done")&&nm.equals("Chicken Burger")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
					
				}
				else if(s.equals("not done")&&nm.equals("Beef Burger")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
					
				}
				else if(s.equals("not done")&&nm.equals("Peperoni Pizza")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
					
				}
				else if(s.equals("not done")&&nm.equals("Cheese Pizza")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
					
				}
				else if(s.equals("not done")&&nm.equals("Sandwich")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
					
				}
				else if(s.equals("not done")&&nm.equals("Set Menu 01")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
					
				}
				else if(s.equals("not done")&&nm.equals("Set Menu 02")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
					
				}
				else if(s.equals("not done")&&nm.equals("Coke")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
					
				}
				else if(s.equals("not done")&&nm.equals("Coffee")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
					
				}
				else if(s.equals("not done")&&nm.equals("Water")&&id==2)
				{
					query2= "UPDATE `order food` SET `status`='done' WHERE `id`=2;";
					st.executeUpdate(query2);
